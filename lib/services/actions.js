...
function deployUpdate (host, service, version, cb) {
  ...
  ssh-add " + host.key + ";\
  git push -f " + host.user + "@" + host.name + ":" + host.root + "/" + service.name +"/repo --all;\
  git push -f " + host.user + "@" + host.name + ":" + host.root + "/" + service.name +"/repo --tags;\
  "
...
function deployInstall (host, service, version, cb) {
  var home = host.root + "/" + service.name
  cmd = "ssh -o StrictHostKeychecking=no -i " + host.key + " " + host.user + "@" + host.name + " '\
  cd " + home + "/versions;\
