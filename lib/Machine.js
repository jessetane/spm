...
Machine.prototype.status = function(services, cb) {
  ...
  do\
    ...
    then\
      if [ -z $(ls $SERVICE) ];\
      then\
        echo $SERVICE\" no versions installed\";\
      else\
...
Machine.prototype.runHook = function(name, service, cb) {
  var self = this
  if (service.hooks && service.hooks[name]) {
    var cmd = service.hooks[name]
...
Machine.prototype.command = function(command, service, cb) {
  ...
  var cmd = this.ssh() + " -t -t 'cd " + service + '; ' + command + "'"
  exec(cmd, cb)
}
...
Machine.prototype.ssh = function(user) {
  if (!user) user = this.user
  var cmd = 'ssh -o StrictHostKeychecking=no ' + user.name + '@' + this.address + ' '
  if (user.key) cmd += '-i ' + user.key + ' '
  return cmd
}
